...
data = f.readlines()
try:
    n = int(input("The file has 50 sudoku's please enter which one to solve (1-46): "))
except:
    print("Invalid input")
    sys.exit()
if n > 46 or n < 1:
...
for i in empty_rows:
    for j in empty_rows[i]:
        l[int(i)][int(j)] = {"1", "2", "3", "4", "5", "6", "7", "8", "9"} - set(row(i)) - set(col(j)) - set(box(int(i), int(j)))
...
def display(d):
    for i in range(0, 9):
        if i % 3 == 0:
            print("  -------------------------------")
        print("", " ".join(d[i][:3]), " ".join(d[i][3:6]), " ".join(d[i][6:9]), sep="  |  ", end="  |\n")
    print("  -------------------------------\n\n")
